...
namespace rend
{
	...
	Rendering::~Rendering(){}
	void Rendering::PosAndScaleToVerteces(attribute::Vector3 pos, attribute::Vector3 scale, std::vector<GLfloat>& verteces, bool x, bool y, bool z, float index)
	{
	...
	void Rendering::PosAndScaleToVerteces(attribute::Symbol* obj, std::vector<GLfloat>& verteces, std::vector<GLuint>& texturesID)
	{
		...
		{
			attribute::Vector3 pos = obj->getPosition();
			...
			attribute::Vector3 posX(obj->getPosition()), posY(obj->getPosition()), posZ(obj->getPosition());
	...
	void Rendering::ParseTreeForTextures(attribute::Symbol* tree)
	{
		Textures["window"] = 1;
		Textures["door"] = 2;
		Textures["wall"] = 3;
		/*std::vector<attribute::Symbol*> queue;
		queue.push_back(tree);

		while (queue.size()){
		attribute::Symbol* cur_item = queue[0];
		...
		attribute::Symbol *child = cur_item->Children[i];
		queue.push_back(child);
		}
		}*/
	}
	void Rendering::ComputeMatricesFromInputs(){
	...
	void Rendering::ActivateTextures(int index)
	{
		switch (index)
		{
	...
	void Rendering::SetTextures(std::map<int, GLuint>& TextureBMP, std::map<int, GLuint>& TextureID)
	{
	...
	GLuint Rendering::LoadShaders(const char * vertex_file_path, const char * fragment_file_path)
	{
		// Create the shaders
	...
	bool Rendering::Render(attribute::Symbol* tree)
	{
		// --------------INITIALIZE GLFW ------------------------------------------------------
		if (!glfwInit())
		{
		...
		window = glfwCreateWindow(Resolution.first, Resolution.second, "Processing Building", NULL, NULL);
		if (window == NULL){
		...
		GLuint programID = LoadShaders("../ProcessingBuildingModel2016/Sources/VertexShader.vertexshader",
			"../ProcessingBuildingModel2016/Sources/FragmentShader.fragmentshader");


		vector<GLfloat> g_vertex_buffer_data;
		vector<GLuint> g_vertex_texture_buffer_data;
		vector<attribute::Symbol*>queue;
		queue.push_back(tree);
		...
		do
		{
			...
			if (IsInitializing || glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS)
			{
				...
				for (iter = Textures.begin(); iter != Textures.end(); ++iter)
				{
					...
					std::string FilePath = "../ProcessingBuildingModel2016/Resources/TextureMix" + TextureMix + "/" + iter->first + ".bmp";
			...
			if (IsInitializing || glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
			{
				...
				if (!IsLastStageRendered)
				{
					...
					vector<attribute::Symbol*> newqueue;
