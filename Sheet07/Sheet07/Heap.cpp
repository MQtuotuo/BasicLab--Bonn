...
double Heap::top() const{ // returns the smallest element
    if (head != NULL){
        return head->value;
    }
    else{
...
HeapElement* Heap::find_leaf(){   // find some leaf (element without children)
...
HeapElement* Heap::insert_as_leaf(double d){// insert a value as leaf, return the element
...
void Heap::order(HeapElement* p)
{
    ...
    {
        cout<<p->value<<" ";
        if(p->child1) order(p->child1);
        if(p->child2) order(p->child2);
...
void repair_downwards(HeapElement* parent){// assure the heap property downwards
    ...
    while (parent->child1 != NULL || parent->child2 != NULL){ //if it has child
        if (parent->child1 != NULL){
            small_child = parent->child1;
            if (parent->child2 != NULL && parent->child2->value < parent->child1->value){ //if it has two child
